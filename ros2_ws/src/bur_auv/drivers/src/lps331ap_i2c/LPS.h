...
// The interface uses a 7-bit number for the address,
...
public:
  ...
  LPS(int pigpio_id);
  ~LPS();
  bool init(deviceType device = device_auto, char sa0 = sa0_auto);
...
private:
  ...
  char address;
  // required for pigpiod i2c methods
  int pi;
  int handle;
